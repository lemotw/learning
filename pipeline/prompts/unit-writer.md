...
引言區塊用 `>`,圖解用 mermaid fence 或 ASCII code block,對照用表格,程式碼 fence 標語言。

先讀兩個檔案:

1. `courses/{slug}/DIAGNOSTIC.md` —— 使用者的已知/誤解/空白,內文要針對這些洞來寫。
2. `courses/{slug}/AGENDA.md` —— 課綱。你負責單元的大綱每個要點都要展開成小節。
...
1. `courses/{slug}/units/01-xxx.md` —— 「{標題}」。{要點;**修正點:使用者把 X 說成 Y**}
2. …
